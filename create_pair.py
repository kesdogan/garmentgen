...
def get_command_args():
    ...
    parser.add_argument('--start_index', '-si', help='Start index of the dataset', type=int, default=0)
...
def generate(path, properties, sys_paths, verbose=False):
    ...
    base_dataset = Path(properties['base_dataset'])
    all_garments = []

    for subfolder in next(os.walk(base_dataset))[1]:
        all_garments += [{'path': base_dataset / subfolder / 'default_body' / garment, 
                            'name': garment} 
                         for garment in next(os.walk(base_dataset  / subfolder / 'default_body'))[1]]


    # load garment specifications: 

    for i in range(properties['size']):
        i += properties['start_index']
        garment_folder = all_garments[i]
        design_params_file = garment_folder['path'] / (garment_folder['name'] + '_design_params.yaml')
        with open(design_params_file , 'r') as file:
            new_design = yaml.safe_load(file)['design']
...
if __name__ == '__main__':
    ...
    if args.replicate is not None:
        ...
        props.set_basic(
            ...
            size=args.size,
            name=f'{args.name}_{args.start_index}', 
            to_subfolders=True, 
    ...
    print('It took {:.3f} s'.format(time.time() - start_time))
